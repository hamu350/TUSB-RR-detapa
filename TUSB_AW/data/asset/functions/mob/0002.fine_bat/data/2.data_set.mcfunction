#> asset:mob/0002.fine_bat/data/2.data_set
...
### 体力等nbtを設定
# 体力
data modify storage asset: mob.Health set value 8
# 緩衝体力
...
# 名前
data modify storage asset: mob.CustomName set value '{"text":"ゲンキコウモリ"}'
# 名前を表示するか
...
### Attributes
data modify storage asset: mob.Attributes append value {Name:generic.max_health, Base:8}
...
# item replace entity @e[tag=ItemHolder,limit=1] armor.legs with diamond_leggings{Unbreakable:true} 1
 item replace entity @e[tag=ItemHolder,limit=1] armor.feet with leather_boots{Unbreakable:true} 1
# 手にも持たせます
# 防具と同様に/lootか/itemで
#item replace entity @e[tag=ItemHolder,limit=1] weapon.mainhand with 
...
#effect一覧
    ...
    #{Name:darkness,id:33}
data modify storage asset: mob.ActiveEffects append value {Id:10,Amplifier:127,Duration:-1,ShowParticles:0b}
...
data modify storage asset: mob set value {Duration:32767,WatiTime:0,Particle:"composter"}
data modify storage asset: mob.Effects set value [{Id:7,Amplifier:0,Duration:4}]
data modify storage _: _.Passengers append from storage asset: mob
