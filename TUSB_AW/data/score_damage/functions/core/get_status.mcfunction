...
# 耐性エフェクト
    execute if data storage score_damage: Argument{BypassResistance:0b} store result score $Resistance ScoreDamageCore run data get entity @s ActiveEffects[{id:"minecraft:resistance"}].Amplifier
    execute if data storage score_damage: Argument{BypassResistance:0b} if data entity @s ActiveEffects[{id:"minecraft:resistance"}] run scoreboard players add $Resistance ScoreDamageCore 1
