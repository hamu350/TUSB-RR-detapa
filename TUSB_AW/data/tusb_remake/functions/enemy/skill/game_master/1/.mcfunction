...
execute as @e[limit=1,tag=GM] at @s run tellraw @a[distance=..60] {"translate": "*§0kk§r§4GameMaster§0§kk§r 後１枚だね","bold": true,"hoverEvent": {"action": "show_text","contents": "ココをクリック！"},"clickEvent": {"action": "run_command","value": "/trigger UNO set 1"}}
execute as @e[limit=1,tag=GM] at @s run tellraw @a[distance=..60] {"translate": "↑をクリックしろ！！！","bold": true}
