...
execute if score _ blast matches 1.. as @e[limit=1,tag=GM] at @s run particle minecraft:explosion ~ ~ ~ 3 3 3 0.1 200 force
...
execute if score _ blast matches 1.. run data modify storage score_damage: Argument set value {Damage:300.00,DamageType:"Projectile"}
execute if score _ blast matches 1.. as @a at @e[limit=1,tag=GM] if entity @s[distance=..6] run function score_damage:api/attack
