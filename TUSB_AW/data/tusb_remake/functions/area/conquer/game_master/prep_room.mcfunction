...
execute as @s[tag=GameMasterArea] if score _ TUSB matches 30.. unless data entity @s Inventory[0] run tp @s -2957 210 -33
# 敵、アイテム削除
execute as @s[tag=GameMasterArea] if score _ TUSB matches 30.. unless data entity @s Inventory[0] unless data storage tusb_remake: game_master{battle:1b} as @e[type=armor_stand,tag=Boss_MarkerC] at @s run tag @e[tag=Enemy,distance=..60] add Garbage
execute as @s[tag=GameMasterArea] if score _ TUSB matches 30.. unless data entity @s Inventory[0] unless data storage tusb_remake: game_master{battle:1b} as @e[type=armor_stand,tag=Boss_MarkerC] at @s run kill @e[type=item,distance=..60]
